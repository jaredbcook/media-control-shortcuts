        ...
        mutation.addedNodes.forEach((node) => {
            if (node.nodeType === 1) { // ELEMENT_NODE
                if (node.tagName === 'VIDEO' || node.tagName === 'AUDIO') {
                    initializeMedia(node as HTMLMediaElement);
                    if (!listenersActive) activateListeners();
                } else if (node.tagName === 'IFRAME') {
                    if (!listenersActive) activateListeners();
                } else {
                    // Check inside added containers
                    if ((node as Element).shadowRoot || ((node as Element).querySelector && ((node as Element).querySelector('video, audio') || (node as Element).querySelector('iframe')))) {
                        const media = findMedia(node as Node);
                        media.forEach(element => initializeMedia(element));
 ...
 */
function getSupportedKeys(): string[] {
 ...
 */
function mapKeyToAction(key: string): string | null {
    switch (key) {
 ...
 */
function mapActionToChange(action: string): number {
    switch (action) {
 ...
 */
function mapActionToSeekPercentage(action: string): number | null {
    switch (action) {
 ...
 * @param {MediaAction} action The action to perform
 */
function runAction(media: HTMLMediaElement | null, action: MediaAction): void {
    if (!media) return;
    if (action.type === ACTION_PLAY_PAUSE) {
        ...
        if (action.seekPercentage !== undefined && !isNaN(media.duration)) {
            media.currentTime = media.duration * action.seekPercentage;
        }
    }
    // Note: ACTION_NEXT_FRAME and ACTION_PREV_FRAME are not defined as constants
    // but are referenced in the original code. They are not currently used.
}
 ...
 */
function findMedia(root: Node | null): (HTMLVideoElement | HTMLAudioElement)[] {
    if (!root) return [];
    let media = Array.from((root as Document | Element).querySelectorAll('video, audio')) as (HTMLVideoElement | HTMLAudioElement)[];
 ...
 */
function initializeMedia(media: HTMLMediaElement | null): void {
    if (!media) return;
 ...
 */
function setMediaPlaybackSpeed(media: HTMLMediaElement | null, speed: number = 1): void {
    if (!media) return;
 ...
 * @param {HTMLMediaElement} media The media element
 */
function toggleMediaPlay(media: HTMLMediaElement | null): void {
    if (!media) return;
    if (media.paused) {
        media.play();
    } else {
 ...
 * @param {HTMLMediaElement} media The media element
 */
function toggleMediaMute(media: HTMLMediaElement | null): void {
    if (!media) return;
    if (media.muted) {
        media.muted = false;
    } else {
 ...
 */
function setMediaVolume(media: HTMLMediaElement | null, volume: number = 1): void {
    if (!media) return;
 ...
 * @param {HTMLMediaElement} media The media element
 * @param {MediaAction} action The action performed on the media element
 */
function showOverlay(media: HTMLMediaElement, action: MediaAction = {}): void {
